...
// d[x]: d[k] + 1 
// k는, arr[x] 보다 arr[k]가 작지만 d[k]가 가장 큰 것) -> 기존엔 O(n)이 었지만, 개선해야 함
// arr[j] < arr[i] 인 것 중에서 최대를 찾는다.
// 오름차순이 유지된다 -> 모노톤 스택을 생각해보자.
// 값을 오름차순으로 저장하고, 인덱스를 길이로 사용한다 -> log 스케일로 탐색이 가능해짐
